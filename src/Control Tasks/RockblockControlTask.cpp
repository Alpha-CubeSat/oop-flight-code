...
void RockblockControlTask::dispatch_process_mo_status()
{
    if (sfr::rockblock::commas[0] == 1 &&
        (sfr::rockblock::buffer[0] == '0' ||
         sfr::rockblock::buffer[0] == '1' ||
         sfr::rockblock::buffer[0] == '2' ||
         sfr::rockblock::buffer[0] == '3' ||
         sfr::rockblock::buffer[0] == '4')) {
        // nominal
...
void RockblockControlTask::get_valid_signal(rockblock_mode_type good_signal, rockblock_mode_type bad_signal)
{
    if (sfr::rockblock::serial.read() == '+' &&
        ...
        char signal = 5; //CHANGE 1 (BENSON): HARDCODED SIGNAL TEST. REMEMBER TO PLUG ANTENNA OUT!
#ifdef VERBOSE
