...
void RockblockControlTask::dispatch_send_message(){
    ...
    for (size_t i=0; i < constants::rockblock::packet_size; ++i){
            if(sfr::rockblock::camera_downlink_period == false){
            #ifdef VERBOSE
            ...
            #ifdef VERBOSE
            Serial.print(sfr::rockblock::camera_report[i]);
            #endif
