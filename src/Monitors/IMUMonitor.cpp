...
void IMUMonitor::execute()
{
    if (sfr::imu::mode == (uint16_t)sensor_mode_type::init) {
        /* IMPORTANT NOTE:
        The IMUMonitor::IMU_setup() cannot be called in the constructor! This is likely (hopefully?)
        due to a constructor trying to call a class method when the object is still being created.
